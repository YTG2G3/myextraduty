...
export default function Nav({ complete: actionComplete }: { complete: Function }) {
    ...
    async function complete() {
        ...
        setLoading(true);
        let res = await actionComplete({ ...JSON.parse(basic), ...JSON.parse(plan), ...JSON.parse(advanced) });
        setLoading(false);
...
function CompBtn({ complete, loading }: { complete: () => void, loading: boolean }) {
    return (
        <Button variant="default" disabled={loading} onClick={complete}>
            {loading ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle className="mr-2" />}
            Complete
        </Button>
    );
}